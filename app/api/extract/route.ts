...
export async function POST(req: NextRequest) {
  ...
  const system = [
    ...
    `- ALL-DAY: if the event has no specific time and lasts the whole day (종일, 하루종일, 온종일, all day, all-day) or is naturally day-long (휴가, 기념일, 생일, holiday, 여행 with no time), set all_day=true. Otherwise all_day=false.`,
    `- When all_day=true, set start_datetime to 00:00 and end_datetime to 23:59 of the SAME date (the last day for multi-day spans), keeping the timezone offset.`,
    `- If no end time is stated (and not all-day), default to start + 1 hour.`,
